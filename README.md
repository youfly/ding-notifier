...
6. config/alert.json 或其他模板文件中，现在可以直接使用以下变量：
   - {{ _now }}: 当前时间 (格式: 2026-08-21 22:30:00)
   - {{ _date }}: 当前日期 (格式: 2026-08-21)
   - {{ _time }}: 当前时间 (格式: 22:30:00)
   - {{ _timestamp }}: Unix 时间戳 (整数)
   - json_escape 函数转义为 JSON 字符串内容（不含外层引号），防止引号/反斜杠破坏 JSON
   - format_time 的几种用法：
### 时间格式化示例

| 语法 (Syntax) | 渲染效果 (Output) | 说明 (Description) |
| :--- | :--- | :--- |
| `{{ timestamp \| format_time }}` | `2026-08-22 11:59:47` | 默认格式 (YYYY-MM-DD HH:mm:ss) |
| `{{ timestamp \| format_time('%Y-%m-%d %H:%M') }}` | `2026-08-22 11:59` | 自定义格式 (年月日时分) |
| `{{ timestamp \| format_time('%Y年%m月%d日 %H:%M') }}` | `2026年08月22日 11:59` | 中文年月日格式 |
| `{{ timestamp \| format_time('%Y-%m-%d %H:%M:%S', 8) }}` | `2026-08-22 19:59:47` | 强制转东八区 (UTC+8) |

> 📌 **注意**：以上输入基准时间均为 UTC 时间：`2026-08-22T11:59:47.690Z`。在强制转为东八区时，时间已自动加上 8 小时。

   - 如果客户请求的参数既不是json格式，也不是form表单格式，那整体数据将当作纯文本处理，在模板里可以通过raw_text变量来引用。
## 快速开始
